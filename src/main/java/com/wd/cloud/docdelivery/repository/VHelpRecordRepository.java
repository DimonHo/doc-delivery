...
public interface VHelpRecordRepository extends JpaRepository<VHelpRecord, Long>, JpaSpecificationExecutor<VHelpRecord> {
    ...
    List<VHelpRecord> findBySendAndGmtModifiedBeforeAndStatusIn(boolean isSend,Date gmtModified,List<Integer> status);
    ...
    @Query(value = "select * from v_help_record where status in (0,1,3) and ((gmt_create between ?1 and ?3 and is_difficult = 0) or (is_difficult = 1 and gmt_modified between ?2 and ?3))", nativeQuery = true)
    Page<VHelpRecord> findByWaitHelp(Date begin1, Date begin2, Date end, Pageable pageable);

    @Query(value = "select * from v_help_record where org_flag = ?4 and status in (0,1,3) and ((gmt_create between ?1 and ?3 and is_difficult = 0) or (is_difficult = 1 and gmt_modified between ?2 and ?3))", nativeQuery = true)
